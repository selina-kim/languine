...
describe("Deck CRUD Operations", () => {
  test("should create new deck with optimistic update", async () => {
    const mockCreateDeck = jest.fn(
      async (deckData: { deck_name: string; language: string }) => {
  ...
  test("should update deck with optimistic update", async () => {
    const mockUpdateDeck = jest.fn(
      async (deckId: string, updates: { deck_name: string }) => {
        return {
          d_id: deckId,
          ...updates,
        };
  ...
  test("should rollback optimistic update on error", async () => {
    let decks = [{ d_id: "1", deck_name: "Original Deck" }];
...
describe("Card CRUD Operations", () => {
  test("should create new card in deck", async () => {
    const mockCreateCard = jest.fn(
      async (deckId: string, cardData: { front: string; back: string }) => {
        return {
          c_id: "card_" + Date.now(),
          d_id: deckId,
          ...cardData,
  ...
  test("should update card content", async () => {
    const mockUpdateCard = jest.fn(
      async (cardId: string, updates: { back: string }) => {
        return {
          c_id: cardId,
          ...updates,
        };
